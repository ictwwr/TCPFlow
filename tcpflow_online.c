...
/*Global variables*/
char * INTERFACE_NAME;
char endtime[STRSIZE];
int totalbyte=0,totalpacket=0;
FILE *fp1,*fp2;
...
void analysis(u_char *agrs, const struct pcap_pkthdr *header,const u_char *packet);	

int main(int argc,char *argv[])
{
	char errBuf[PCAP_ERRBUF_SIZE], *devstr;
	
	char  *query_str,starttime[20],starttime1[20];
	char t[20],tableid[30]; 
	int rc,totalflow;
	pcap_t *fd;
	record *s, *tmp, *p;
	src *s1, *tmp1, *p1;
 	...
 	/*Find available devices*/
    	...
    	devstr=INTERFACE_NAME;				

    	/* Open the network card in a hybrid mode  */
   	fd = pcap_open_live(devstr, MAXLINE, 1, 0, errBuf);
	...
	pcap_loop(fd, count, analysis, (u_char *)&id);
	...
	fprintf(fp2,"StartTime:%s %s,EndTime:%s,",starttime,starttime1,endtime);
	
	/*Total number of TCPflows*/
	totalflow=HASH_COUNT(records);
	fprintf(fp2,"TotalFlows:%d,TotalPacket:%d,TotalByte:%d\n\n", totalflow,totalpacket,totalbyte);
	...
	fprintf(fp2,"\nThe Top 10 of TCPFlows by Packets:\n");
	for ( k = 1; k <= n; k++)   
	{
		j=index[k];
...
static int parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argvopt, "i:c:w:s:")) != -1) {
...
void analysis(u_char *agrs, const struct pcap_pkthdr *header,const u_char *packet)
{
	int time,size_ip,size_tcp,size_packet,len;
	char capture_time[STRSIZE];
	char dp[15],sp[15],si[100],di[100],id[20];
	struct iphdr *ip_header;
	struct tcphdr *tcp_header;
	char  *query_str;
	int rc,t,i;
	long long int rows,index;
	char p[10],query[65535];
	
	len=header->len;
	time=header->ts.tv_sec;
	size_packet=header->caplen;
	//Convert the packet capture time to local time
	strftime(capture_time,sizeof(capture_time),"%Y-%m-%d %T",localtime(&(header->ts.tv_sec)));	
	
	ip_header=(struct iphdr *)(packet+SIZE_ETHERNET);
	size_ip=(ip_header->ihl)*4;
	
	//Determine TCP according to the protocol number 
	if(ip_header->version==0x04&&ip_header->protocol==IPPROTO_TCP)
	{
